...
- [ ] **1.5.1** 验证：未登录访问 `/` 重定向到 `/login` `[e2e]`
- [ ] **1.5.2** 验证：注册新用户 → 自动登录 → 跳转到首页 `[e2e]`
- [ ] **1.5.3** 验证：登录已有用户 → 跳转到首页，tRPC 请求正常 `[e2e]`
- [ ] **1.5.4** 验证：错误密码登录 → 显示通用错误 `[e2e]`
- [ ] **1.5.5** 验证：Socket.io 连接在登录后正常建立（cookie 认证） `[e2e]`
- [ ] **1.5.6** 验证：DEV_MODE=true 时，快速登录功能正常 `[e2e]`
