...
interface StateRestrictionResult {
  blocked: boolean;
  redirectUrl?: string;
  reason?: 'locked' | 'over_rest';
}

interface RestrictionReason {
  reason: 'locked' | 'over_rest' | null;
  message: string;
}
...
function shouldBlockForStateRestriction(
  ...
  return {
    blocked: true,
    redirectUrl: `chrome-extension://test-extension-id/${screensaverPath}`,
    reason,
  };
}
...
const nonRestrictedStateArbitrary = fc.constantFrom<SystemState>('PLANNING', 'FOCUS', 'REST');
const restrictedStateArbitrary = fc.constantFrom<SystemState>('LOCKED', 'OVER_REST');
const allStatesArbitrary = fc.constantFrom<SystemState>('LOCKED', 'PLANNING', 'FOCUS', 'REST', 'OVER_REST');
...
describe('Property 1: LOCKED State Dashboard Restriction', () => {
  /**
   * Property: In LOCKED state, only Dashboard URLs are allowed
   * For any browser navigation attempt when system state is LOCKED 
   * (Airlock not completed today), the Browser Sentinel SHALL redirect 
   * to Dashboard, regardless of whether current time is within configured work hours.
   * Validates: Requirements 1.1, 1.2, 1.10
   */

  it('should block all non-Dashboard URLs when in LOCKED state', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (domain, path) => {
          ...
          const result = shouldBlockForStateRestriction(url, 'LOCKED');
          
          // Property: Non-Dashboard URLs should be blocked in LOCKED state
          expect(result.blocked).toBe(true);
          expect(result.reason).toBe('locked');
          expect(result.redirectUrl).toContain('locked-screensaver.html');
  ...
  it('should allow Dashboard URLs when in LOCKED state', async () => {
    await fc.assert(
      fc.asyncProperty(
        pathArbitrary,
        async (path) => {
          ...
          const result = shouldBlockForStateRestriction(url, 'LOCKED', dashboardUrl);
          
          // Property: Dashboard URLs should be allowed in LOCKED state
  ...
  it('should block regardless of work time configuration in LOCKED state', async () => {
    // This test verifies that LOCKED state blocking is independent of work time
    // The key requirement is that LOCKED state blocks regardless of work hours
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom(...nonDashboardDomains),
        pathArbitrary,
        fc.boolean(), // Simulates whether within work time or not
        async (domain, path, _isWithinWorkTime) => {
          const url = `https://${domain}${path}`;
          
          // Property: LOCKED state should block regardless of work time
          // The shouldBlockForStateRestriction function doesn't check work time
          // because LOCKED state restriction applies unconditionally
          const result = shouldBlockForStateRestriction(url, 'LOCKED');
  ...
  it('should return correct restriction message for LOCKED state', () => {
    const reason = getRestrictionReason('LOCKED');
    
    expect(reason.reason).toBe('locked');
    expect(reason.message).toBe('请完成今日计划');
  });

  it('should allow internal URLs in LOCKED state', async () => {
    const internalUrls = [
      'chrome://extensions',
...
describe('State Restriction - Non-Restricted States', () => {
  /**
   * Property: Non-restricted states (PLANNING, FOCUS, REST) should not 
   * trigger state-based blocking
   */

  it('should not block any URLs in PLANNING state', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom(...nonDashboardDomains),
        pathArbitrary,
        async (domain, path) => {
          const url = `https://${domain}${path}`;
          const result = shouldBlockForStateRestriction(url, 'PLANNING');
          
          // Property: PLANNING state should not trigger state restriction
          expect(result.blocked).toBe(false);
  ...
  it('should not block any URLs in REST state', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom(...nonDashboardDomains),
        pathArbitrary,
        async (domain, path) => {
          const url = `https://${domain}${path}`;
          const result = shouldBlockForStateRestriction(url, 'REST');
          
          // Property: REST state should not trigger state restriction
          expect(result.blocked).toBe(false);
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (state) => {
          ...
          // Property: Only LOCKED and OVER_REST are restricted states
          if (state === 'LOCKED' || state === 'OVER_REST') {
...
describe('State Transition Scenarios', () => {
  ...
  it('should restore normal browsing when transitioning from LOCKED to PLANNING', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom(...nonDashboardDomains),
        pathArbitrary,
        async (domain, path) => {
          ...
          // Before: LOCKED state blocks
          const lockedResult = shouldBlockForStateRestriction(url, 'LOCKED');
          expect(lockedResult.blocked).toBe(true);
          
          // After: PLANNING state allows
          const planningResult = shouldBlockForStateRestriction(url, 'PLANNING');
          expect(planningResult.blocked).toBe(false);
  ...
  it('should block when transitioning from REST to OVER_REST', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom(...nonDashboardDomains),
        pathArbitrary,
        async (domain, path) => {
          ...
          // Before: REST state allows
          const restResult = shouldBlockForStateRestriction(url, 'REST');
          expect(restResult.blocked).toBe(false);
...
describe('Redirect URL Correctness', () => {
  ...
  it('should redirect to locked-screensaver.html for LOCKED state', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom(...nonDashboardDomains),
        pathArbitrary,
        async (domain, path) => {
          ...
          expect(result.blocked).toBe(true);
          expect(result.redirectUrl).toContain('locked-screensaver.html');
          expect(result.redirectUrl).not.toContain('over-rest-screensaver.html');
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (domain, path) => {
          ...
          expect(result.blocked).toBe(true);
          expect(result.redirectUrl).toContain('over-rest-screensaver.html');
          expect(result.redirectUrl).not.toContain('locked-screensaver.html');
