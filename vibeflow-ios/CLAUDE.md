...
所有 iOS 开发命令统一通过项目根目录的 `dev.sh` 执行：

```bash
cd /path/to/vibeflow  # 项目根目录

# 最常用：Web 后端 + iOS Metro (JS 热更新，需要设备已安装 Dev Client)
./scripts/dev.sh ios

# 编译原生代码后部署 (智能增量，30-60s)
./scripts/dev.sh ios --build

# 强制重建 ios/ 目录 (首次或改 native module 时用，2-5min)
./scripts/dev.sh ios --build --clean

# Release 构建 (独立运行，不需要 Metro)
./scripts/dev.sh ios --release

# 只启动 iOS，不启动本地后端 (连远程服务器时用)
./scripts/dev.sh ios --remote
./scripts/dev.sh ios --only
```

### Expo Prebuild 详解

`npx expo prebuild` 根据 `app.config.ts` 生成原生 `ios/` 目录（Xcode 项目）。

| 模式 | 命令 | 耗时 | 适用场景 |
|------|------|------|----------|
| 增量 | `npx expo prebuild --platform ios` | 10-30s | 日常开发、JS 代码变更 |
| 完全重建 | `npx expo prebuild --clean --platform ios` | 2-5min | 添加/删除 native module、改 bundle ID、升级 SDK、诡异构建错误 |

**何时需要 `--clean`**：
- 添加/删除 native module（如 `expo install expo-camera`）
- 修改 `app.config.ts` 中的 native 配置（bundleIdentifier、权限、entitlements）
- 修改 `plugins/` 目录下的 Config Plugin
- 升级 Expo SDK 版本
- 遇到诡异的 native 构建错误（"Clean Build" 万能解法）
